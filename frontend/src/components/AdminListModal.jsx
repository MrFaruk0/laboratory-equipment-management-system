    ...
    >
      <div
        style={{
          ...
          maxWidth: "900px", width: "90%", maxHeight: "80vh",
      ...
      >
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "24px" }}>
          {loading && <p style={{ color: "#9ca3af", textAlign: "center" }}>{t("modal.loading")}</p>}
          {error && <p style={{ color: "#dc2626", textAlign: "center" }}>{error}</p>}

          {!loading && !error && items.length === 0 && (
            <p style={{ color: "#9ca3af", textAlign: "center" }}>{t("modal.noItems")}</p>
          )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
              <thead>
                <tr style={{ borderBottom: "2px solid #e5e7eb" }}>
                  {columns.map((col) => (
                    <th key={col} style={{ textAlign: "left", padding: "12px 8px", fontWeight: "600", color: "#6b7280" }}>
                      {col}
                    </th>
                  ))}
                </tr>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #e5e7eb", textAlign: "right" }}>
          <button onClick={onClose} style={{ background: "#f3f4f6", border: "1px solid #d1d5db", borderRadius: "8px", padding: "8px 16px", cursor: "pointer", fontSize: "14px", fontWeight: "500" }}>
            {t("modal.close")}
          </button>
        </div>
      </div>
